...
void kvm_run_vm(struct vm *vm1, struct vm *vm2)
{
    ...
    while (!vm1_done || !vm2_done)
    {
        ...
        case KVM_EXIT_IO:
            printf("VMFD: %d KVM_EXIT_IO\n", vm->vm_fd);
            if (vm->vcpus->kvm_run->io.direction == KVM_EXIT_IO_OUT && vm->vcpus->kvm_run->io.port == 0x10)
            {
                buffer[produce_pointer] = *(int *)((char *)(vm->vcpus->kvm_run) + vm->vcpus->kvm_run->io.data_offset);
                printf("VMFD: %d Produced value: %d\n", vm->vm_fd, buffer[produce_pointer]);
                produce_pointer = (produce_pointer + 1) % 3;
                if (produce_pointer == 0)
                    current_vcpu = 2;
            }
            else if (vm->vcpus->kvm_run->io.direction == KVM_EXIT_IO_IN && vm->vcpus->kvm_run->io.port == 0x11)
            {
                *(int *)((char *)(vm->vcpus->kvm_run) + vm->vcpus->kvm_run->io.data_offset) = buffer[consume_pointer];
                consume_pointer = (consume_pointer + 1) % 3;
            }
            else if (vm->vcpus->kvm_run->io.direction == KVM_EXIT_IO_OUT && vm->vcpus->kvm_run->io.port == 0x12)
            {
                printf("VMFD: %d Consumed value: %d\n", vm->vm_fd, *(int *)((char *)(vm->vcpus->kvm_run) + vm->vcpus->kvm_run->io.data_offset));
                if (consume_pointer == 0)
                {
                    current_vcpu = 1;
                }
            }
