...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
    ...
    uint32_t exits = 0;
    uint32_t input_in = 0;
    uint32_t input_out = 0;

    uint32_t hva_global = 0;
    ...
    for (;;)
    {
        ...
        case KVM_EXIT_IO:
            ...
            else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEC)
            {
                ...
                char *hva = (char *)vm->mem + gva;
                fprintf(stdout, "%s", hva);
                fflush(stdout);
                continue;
            }
            ...
            else if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEE)
            {
                ...
                {
                    hva_global = (uint32_t)((uintptr_t)vm->mem + translation.physical_address);
                }
                continue;
            }
